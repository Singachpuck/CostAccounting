...
from rest.dao.impl.inmemory.CategoryDaoInMemory import categories
from rest.dao.impl.inmemory.UserDaoInMemory import users
...
class TransactionDaoInMemory(TransactionDao):
    ...
    def getTransactionsByUserIdAndCategory(self, userId, category) -> list[Transaction]:
        return list(
            filter(lambda transaction: transaction.user.id == userId and transaction.category == category,
                   transactions))

    def createTransaction(self, transaction: Transaction) -> None:
        if transaction.id is None:
            transaction.id = self.__generateId()
...
transactionDao = TransactionDaoInMemory()
